...
public class TournamentHistoryManager {
    ...
    public long recordTournamentEnd(long startedAtMs, int totalRounds) {
        ...
        plugin.getDatabaseManager().runWithConnection(c -> {
            try (PreparedStatement ps = c.prepareStatement("""
                ...
                try (ResultSet keys = ps.getGeneratedKeys()) {
                    if (keys.next()) historyId[0] = keys.getLong(1);
                }
            }
        ...
        if (historyId[0] < 0) return -1;

        // Fire achievement hooks for everyone
        for (int i = 0; i < playersByPoints.size(); i++) {
            PlayerData pd = playersByPoints.get(i);
            int rank = i + 1;
        ...
        plugin.getLogger().info("Tournament recorded — id " + historyId[0]
